...
class AppStatic {
   ...
   * @returns 
   */
  static getStaticAttrs(attrs = {}) {
    let retList = [];
    for (const i in attrs) {
      retList.push(`${i}="${attrs[i]}"`);
    }
  ...
  static getStatic(json, code) {
    ...
    if (css !== undefined && css instanceof Object) {
      for (const c in css) {
        let attrStr = this.getStaticAttrs(css[c].attrs);
        sTops.push(`  <link rel=\"stylesheet\" href=\"${c}\" ${attrStr}/>`);
      }
    }
    let js = json.js;
    if (js !== undefined && js instanceof Object) {
      for (const j in js) {
        let attrStr = this.getStaticAttrs(js[j].attrs);
        let jv = `<script src=\"${j}\"${attrStr}></script>`;
        if (js[j].top) {
          sTops.push("  " + jv);
        } else {
          sBottoms.push(jv);
   ...
   * @returns 
   */
  static getBody(body, json, hd) {
    let hdHandle = hd.ctx.app.handle;
    if (hdHandle.json === undefined) {
      hdHandle['json'] = json;
    } else {
      for (const i in json) {
        if (['js', 'css'].includes(i)) {
          for (const j in json[i]) {
            hdHandle.json[i][j] = json[i][j];
          }
        } else {
          hdHandle.json[i] = json[i];
        }
      }
    }

    let code = hd.data.code;

    if (hdHandle.code === undefined) {
      hdHandle['code'] = code;
    } else {
      for (const i in code) {
        for (const j in code[i]) {
          hdHandle.code[i].push(code[i][j]);
        }
      }
    }

    let isNull = false;
    let bType = tool.type(body);
    if (bType === "Null") {
      body = '';
      isNull = true;
    } else {
