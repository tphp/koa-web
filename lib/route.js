...
class Route {
   ...
   * @param {*} ctx 
   */
  static setCtxType(ctx, ext) {
    if (ctx.type === '' || ctx.type === undefined) {
      if (this.extTypes[ext] === undefined) {
        if (mime.types[ext] !== undefined) {
          ctx.type = mime.types[ext];
        }
      } else {
        ctx.type = this.extTypes[ext];
      }
    }
  }
  ...
  static async app(cUrl, ctx, cache, isApp = true, view = undefined, isError = false, errorCode = 0, errorMessage = '') {
    ...
    if (ext !== '') {
      ...
      if (!ret.isJson && typeof ret.body === 'string') {
        ret.body = appStatic.getBody(ret.body, json, hd);
      }
      ret['ext'] = ext;
    ...
    if (!moduleIsFile && !extIsFile) {
      ...
      let ae = this.appExit(jsData);
      if (!ae.isJson && typeof ae.body === 'string') {
        ae.body = appStatic.getBody(ae.body, json, hd);
      }
      return ae;
    }
    ...
    if (isApp) {
      jsData = await this.appExtCall('html', cBody, hd, hdData, hdFiles);
    } else {
      jsData = cBody;
    }
    ...
    if (typeof jsData === 'string') {
      jsData = appStatic.getBody(jsData, json, hd);
    }
