...
class App {
  ...
  static async app(ctx, next) {
    ...
    // 处理SEO和静态文件
    if (typeof ctx.body === 'string' && ctx.app.handle.json !== undefined && ctx.app.handle.code !== undefined) {
      let stt = appStatic.getStatic(ctx.app.handle.json, ctx.app.handle.code);
      ctx.body = appStatic.getSeo(ctx.body, ctx.app.handle.json, stt.top);
      ctx.body = appStatic.getScript(ctx.body, stt.bottom);
    }
