...
class Handle {
  ...
  hd = {
     ...
     * @returns 
     */
    setStatic: (config, type = 'css', attrs = {}) => {
      if (config === undefined) {
        return;
      }
   ...
   */
  css(config = '', attrs = {}) {
    this.hd.setStatic(config, 'css', attrs);
  }
   ...
   */
  js(config = '', attrs = {}) {
    this.hd.setStatic(config, 'js', attrs);
  }
