...
class Route {
   ...
   * @returns 
   */
  static async __render(viewFile, viewData, viewDir, cache) {
    if (!this.isRender) {
  ...
  static async app(cUrl, ctx, cache, view = undefined, isError = false, errorCode = 0, errorMessage = '') {
    ...
    if (extIsFile) {
      ...
      cBody = await this.__render(hdExtFile, hdView, json.view, cache);
      if (isLayout) {
        hdView['__layout__'] = cBody;
        cBody = await this.__render(hdLayoutFile, hdView, json.view, cache);
      }
    } else {
      cBody = jsData;
    }
