...
class Route {
   ...
   * @returns 
   */
  static async __render(viewFile, viewData) {
    if (!this.isRender) {
      return await this.nunjucks.render(viewFile, viewData);
    }

    const vFile = Route.path + "/" + Route.view.path + "/" + viewFile;
    const html = fs.readFileSync(vFile).toString();
  ...
  static async app(cUrl, ctx, cache, view = undefined, isError = false, errorCode = 0, errorMessage = '') {
    ...
    if (extIsFile) {
      ...
      cBody = await this.__render(hdExtFile, hdView);
      if (isLayout) {
        hdView['__layout__'] = cBody;
        cBody = await this.__render(hdLayoutFile, hdView);
      }
    } else {
      cBody = jsData;
    }
