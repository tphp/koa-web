...
class App {
  ...
  static setConfig(config) {
    ...
    var configStatic = config.static;
    ...
    if (typeof configStatic === 'string') {
      ...
      for (var i in configStatic) {
        var iv = configStatic[i];
        if (typeof iv !== 'string') {
          continue;
        }
        iv = route.dirTrim(iv);
  ...
  static async app(ctx, next) {
    App.setLoad(ctx.app);
    await route.app(ctx.url, ctx, next, App.cache);
    await next();
    if (!ctx.body) {
      ctx.body = await route.getAppError('404 Error!', 404, ctx, next, App.cache);
    }
  }
}
